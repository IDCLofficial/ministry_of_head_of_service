...
export default function ServiceCard({ imgSrc, title, description }: ServiceCardProps) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col p-6 h-full">
      ...
      <h3 className="text-xl md:text-lg font-semibold mb-2 uppercase tracking-wide text-center">{title}</h3>
      <p className="text-dark-primary-body text-base text-center">{description}</p>
    </div>
  );
} 